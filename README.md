...
- **`metadata` is Terraform-authoritative.** After apply, the server's metadata equals the map in your HCL: removing a key from your HCL deletes it server-side, and a key added out-of-band is removed on the next apply. (On the wire the provider sends a JSON merge-patch, not the whole map, but refresh re-reads the full server map so your HCL stays authoritative.) Applies to both `claude-managed-agents_agent.metadata` and `claude-managed-agents_vault.metadata`.
...
```sh
git clone https://github.com/modus-agendi/terraform-provider-anthropic-claude-managed-agents
...
> copies the binary into `~/.terraform.d/plugins/registry.terraform.io/modus-agendi/anthropic-claude-managed-agents/`,
